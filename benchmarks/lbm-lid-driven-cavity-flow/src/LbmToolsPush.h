...
template <typename PopulationField,
          typename LbmComputeType>
struct LbmToolsTemplatePush<D3Q19Template<typename PopulationField::Type, LbmComputeType>,
                            PopulationField,
                            LbmComputeType>
{
...
#define LOADPOP(GOx, GOy, GOz, GOid, BKx, BKy, BKz, BKid) \
    {                                                     \
        { /*GO*/                                          \
            popIn[GOid] = fin(i, GOid);                   \
        }                                                 \
        { /*BK*/                                          \
            popIn[BKid] = fin(i, BKid);                   \
        }                                                 \
    }
    static inline NEON_CUDA_HOST_DEVICE auto
    loadLocalPopulation(Cell const&                                i,
...
#define PUSH_STREAM(GOx, GOy, GOz, GOid, BKx, BKy, BKz, BKid)                                        \
    {                                                                                                \
        { /*GO*/                                                                                     \
               ...
               NEON_OUT LbmStoreType                      popIn[19])
    {
        ...
        PUSH_STREAM(-1, 0, 0, /*  GOid */ 0, /* --- */ 1, 0, 0, /*  BKid */ 10);
        PUSH_STREAM(0, -1, 0, /*  GOid */ 1, /* --- */ 0, 1, 0, /*  BKid */ 11);
        PUSH_STREAM(0, 0, -1, /*  GOid */ 2, /* --- */ 0, 0, 1, /*  BKid */ 12);
        PUSH_STREAM(-1, -1, 0, /* GOid */ 3, /* --- */ 1, 1, 0, /*  BKid */ 13);
        PUSH_STREAM(-1, 1, 0, /*  GOid */ 4, /* --- */ 1, -1, 0, /* BKid */ 14);
        PUSH_STREAM(-1, 0, -1, /* GOid */ 5, /* --- */ 1, 0, 1, /*  BKid */ 15);
        PUSH_STREAM(-1, 0, 1, /*  GOid */ 6, /* --- */ 1, 0, -1, /* BKid */ 16);
        PUSH_STREAM(0, -1, -1, /* GOid */ 7, /* --- */ 0, 1, 1, /*  BKid */ 17);
        PUSH_STREAM(0, -1, 1, /*  GOid */ 8, /* --- */ 0, 1, -1, /* BKid */ 18);
        //  }
        // Treat the case of the center (c[k] = {0, 0, 0,}).
        {
            popIn[Lattice::centerDirection] = fin(i, Lattice::centerDirection);
...
#undef POP
    ...
    static inline NEON_CUDA_HOST_DEVICE auto
    collideBgkUnrolledOnRegisters(Cell const&                          i /*!     LbmComputeType iterator   */,
...
#define COMPUTE_GO_AND_BACK(GOid, BKid)                             \
    {                                                               \
        pop[GOid] = static_cast<LbmStoreType>(pop_out_0##GOid);     \
        pop[BKid] = static_cast<LbmStoreType>(pop_out_opp_0##GOid); \
    }
...
#undef COMPUTE_GO_AND_BACK

        {
            ...
            const LbmComputeType pop_out_09 = (1. - omega) *
                                              ...
                                              omega * eq_09;
            pop[Lattice::centerDirection] = static_cast<LbmStoreType>(pop_out_09);
        }
    }

    static auto
    iterationPush(Neon::set::TransferSemantic stencilSemantic,
            ...
            [&, omega](Neon::set::Loader& L) -> auto {
                ...
                return [=] NEON_CUDA_HOST_DEVICE(const typename PopulationField::Cell& cell) mutable {
                    ...
                    if (cellInfo.classification == CellType::bulk) {
                        ...
                        loadLocalPopulation(cell, fIn, NEON_OUT popIn);
                        ...
                        collideBgkUnrolled(cell,
                                           ...
                                           usqr, omega,
                                           NEON_IO popIn);
                        pushStream(cell, cellInfo.wallNghBitflag, fIn, NEON_OUT popIn);
                    }
                };
            });
        return container;
        ...
        Neon::set::Container container = fInField.getGrid().getContainer(
            ...
            [&](Neon::set::Loader& L) -> auto {
                ...
                return [=] NEON_CUDA_HOST_DEVICE(const typename PopulationField::Cell& cell) mutable {
                    ...
                    if (cellInfo.classification == CellType::bulk) {
                        loadLocalPopulation(cell, cellInfo.wallNghBitflag, fIn, NEON_OUT popIn);
                        macroscopic(popIn, NEON_OUT rho, NEON_OUT u);
                    } else {
