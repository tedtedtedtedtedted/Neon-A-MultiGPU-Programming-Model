...
auto eGrid::getProperties(const Neon::index_3d& idx) const -> GridBaseTemplate::CellProperties
{
    ...
    if (this->getDevSet().setCardinality() == 1) {
        ...
        //TODO
        NEON_DEV_UNDER_CONSTRUCTION("eGrid only support single GPU");
    }
    return cellProperties;
}

auto eGrid::isInsideDomain(const Neon::index_3d& idx) const -> bool
{
    if (this->getDevSet().setCardinality() != 1) {
        NEON_DEV_UNDER_CONSTRUCTION("eGrid only support single GPU");
    }
...
auto eGrid::getOriginBlock3DIndex(const Neon::int32_3d idx) const -> Neon::int32_3d
{
...
auto eGrid::setReduceEngine(Neon::sys::patterns::Engine eng) -> void
{
    if (eng != Neon::sys::patterns::Engine::CUB) {
        NeonException exp("eGrid::setReduceEngine");
        exp << "eGrid only work on CUB engine for reduction";
        NEON_THROW(exp);
    }
}
                                ...
                                const size_t&                          sharedMem) const -> Neon::set::LaunchParameters
{
    if (dataView != Neon::DataView::STANDARD) {
        NEON_WARNING("Requesting LaunchParameters on {} data view but eGrid only supports Standard data view on a single GPU",
...
auto eGrid::getPartitionIndexSpace(Neon::DeviceType dev,
                                   SetIdx           setIdx,
...
auto eGrid::getOrigins() const -> const Neon::set::MemSet_t<Neon::int32_3d>&
{
    return mData->mOrigin;
}

auto eGrid::getStencilNghIndex() const -> const Neon::set::MemSet_t<nghIdx_t>&
{
    return mData->mStencilNghIndex;
}


auto eGrid::getNeighbourBlocks() const -> const Neon::set::MemSet_t<uint32_t>&
{
    return mData->mNeighbourBlocks;
}

auto eGrid::getActiveMask() const -> Neon::set::MemSet_t<uint32_t>&
{
    return mData->mActiveMask;
}

auto eGrid::getBlockOriginTo1D() const -> Neon::domain::tool::PointHashTable<int32_t, uint32_t>&
{
    return mData->mBlockOriginTo1D;
}


auto eGrid::getDimension() const -> const Neon::index_3d
{
    return GridBase::getDimension();
}

auto eGrid::getNumBlocks() const -> const Neon::set::DataSet<uint64_t>&
{
    return mData->mNumBlocks;
}

auto eGrid::getBlockSize() const -> int
{
    return mData->blockSize;
}

auto eGrid::getVoxelSpacing() const -> int
{
