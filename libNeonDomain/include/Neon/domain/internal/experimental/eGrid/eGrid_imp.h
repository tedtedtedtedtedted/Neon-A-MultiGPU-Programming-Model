...
eGrid::eGrid(
    ...
    : eGrid(backend, domainSize, activeCellLambda, stencil, 8, 1, spacingData, origin, bcLambda)
{
}
...
auto eGrid::newField(const std::string          name,
                     int                        cardinality,
                     T                          inactiveValue,
        ...
        *this, [&](Neon::set::Loader& loader) {
            ...
            return [&](int streamIdx, Neon::DataView dataView) mutable -> void {
                if (dataView != Neon::DataView::STANDARD) {
                    ...
                    exc << "Reduction operation on eGrid works only on standard dataview";
