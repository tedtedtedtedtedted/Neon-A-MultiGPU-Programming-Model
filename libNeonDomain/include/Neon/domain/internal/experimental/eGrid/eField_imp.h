...
eField<T, C>::eField(const std::string&             name,
                     const eGrid&                   grid,
                     int                            cardinality,
                     T                              outsideVal,
                                                                             ...
                                                                             haloStatus)
{
    ...
    mData->grid = std::make_shared<eGrid>(grid);
...
template <typename T, int C>
auto eField<T, C>::forEachActiveCell(
...
template <typename T, int C>
auto eField<T, C>::isInsideDomain(const Neon::index_3d& idx) const -> bool
{
    return mData->grid->isInsideDomain(idx);
}

template <typename T, int C>
auto eField<T, C>::getRef(const Neon::index_3d& idx,
...
template <typename T, int C>
auto eField<T, C>::haloUpdate(Neon::set::HuOptions& /*opt*/) const -> void
{
    //TODO
    NEON_DEV_UNDER_CONSTRUCTION("eField::haloUpdate");
}

template <typename T, int C>
auto eField<T, C>::haloUpdate(Neon::set::HuOptions& /*opt*/) -> void
{
    //TODO
    NEON_DEV_UNDER_CONSTRUCTION("eField::haloUpdate");
}
...
template <typename T, int C>
auto eField<T, C>::updateIO(int streamId) -> void
{
...
template <typename T, int C>
auto eField<T, C>::updateCompute(int streamId) -> void
{
...
template <typename T, int C>
auto eField<T, C>::getPartition(Neon::Execution       exec,
                                Neon::SetIdx          idx,
                                const Neon::DataView& dataView) const -> const Partition&
{
    ...
    NEON_THROW_UNSUPPORTED_OPERATION("eField::getPartition() unsupported Execution");
}
...
template <typename T, int C>
auto eField<T, C>::getMem() -> Neon::set::MemSet_t<T>&
{
    return mData->mem;
}

template <typename T, int C>
auto eField<T, C>::getSharedMemoryBytes(const int32_t stencilRadius) const -> size_t
{
